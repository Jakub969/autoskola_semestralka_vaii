...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if(Auth::user()->role == 'instruktor' || Auth::user()->role == 'admin')
                <x-responsive-nav-link :href="route('sessions-view')" :active="request()->routeIs('sessions-view')">
                    {{ __('Prihlasovanie jázd') }}
            ...
            @endif
            @if(Auth::user()->role == 'instruktor' || Auth::user()->role == 'admin')
                <x-responsive-nav-link :href="route('driving-sessions')"
                                       :active="request()->routeIs('driving-sessions')">
                    {{ __('Termíny jazd') }}
                </x-responsive-nav-link>
            @endif
            @if(Auth::user()->role == 'admin')
                <x-responsive-nav-link :href="route('update-car')" :active="request()->routeIs('update-car')">
                    {{ __('Pridanie aut') }}
                </x-responsive-nav-link>
            @endif
            @if(Auth::user()->role == 'admin')
                <x-responsive-nav-link :href="route('update-users')" :active="request()->routeIs('update-users')">
                    {{ __('Úprava užívateľov') }}
                </x-responsive-nav-link>
            @endif
        </div>
